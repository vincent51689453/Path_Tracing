...
DEFAULT_LABELMAP = '/home/nvidia/vincent/workspace/handhygiene_QE_ws/third_party/models/research/object_detection/data/labelmap.pbtxt'
...
def on_message(client, userdata, msg):
    ...
    if(message_dict["status"] == "1"):
        print("message get!")
        hand_wash_status = 1
        with open('./path_analyzer/path_log.csv','a',newline='') as wash_record_file:
            log_writer=csv.writer(wash_record_file)
            if(message_dict["sensor_id"] == "0"):
                #Hand Rub
                log_writer.writerow([int(0),int(452),int(298),int(200),int(0),int(200),int(1),int(0)])
            if(message_dict["sensor_id"] == "2"):
                #Wash Hand
                log_writer.writerow([int(0),int(275),int(348),int(200),int(0),int(200),int(1),int(0)])
...
class BBoxVisualization():
    ...
    def draw_bboxes(self, img, box, conf, cls):
        ...
        for bb, cf, cl in zip(box, conf, cls):
            cl = int(cl)
            if((cl == 1)and(cf>=0.5)): #Vincent:Only process "person" (label id of persion = 1) + confidence control
        ...
        while True:
		    ...
		    # object on the output frame
                    ...
                    with open('./path_analyzer/path_log.csv','a',newline='') as csv_log_file:
                        ...
                        log_writer.writerow([objectID,centroid[0],centroid[1],int(distance_bed),int(distance_clean),int(distance_clean_alchol),int(0),int(0)])
#log_writer.writerow([objectID,centroid[0],centroid[1],int(distance_bed),int(distance_clean),int(distance_clean_alchol),int(hand_wash_status),int(personal_status[objectID])])
                ...
                if((leave_zone_counter==0)and(enter_zone == True)and(leave_zone==False)):
                ...
                if(((person_x > 0)and(person_y > 0))and(leave_zone_counter>0)):
                    font = cv2.FONT_HERSHEY_PLAIN
                    line = cv2.LINE_AA
                    if (enter_zone == False):
                        cv2.putText(img, "START", (40, 100), font, 3.0, (255, 0,0), 4, line)
                        print("START\r\n")
                        enter_zone = True
                        ...
                        if(leave_zone == True):
                            cv2.putText(img, "COUNT", (40, 100), font, 3.0, (255, 0,0), 4, line)
                            print("COUNT\r\n")
